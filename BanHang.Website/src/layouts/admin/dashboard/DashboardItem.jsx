...
import {TopMakeOrder,TopSpendTable} from '../../../components/admin/reportTable/'
import {useDispatch} from "react-redux";
import {
  getAllOrderWithoutPagi, getListProducts,
  getListUsers,
...
import {BestSellingCard, TopSpendCard} from "../../../components/shared/card/AdminDashCard";
...
const DashboardsItem = () => {
  ...
  //  get total Proceeds each Month
    const payload = await unwrapResult(
      await dispatch(getProceedsEachMonth())
    )
    setTotalProceedsEachMonth(payload.result)

    const listUsers = await unwrapResult(
      await  dispatch(getListUsers())
    )
    setListUsers(listUsers)

    const listProducts = await unwrapResult(
  ...
  return (
    <>
      <div className="admin__dashboard">
        ...
        <Row>
          <Col sm={6}>
            {totalProceedsEachMonth&&<BarChart chartData={totalProceedsEachMonth}/>}
          </Col>
          <Col sm={6}>
            {listUsers &&
            <Row className="m-t-30">
              <Col sm={6}>
                <ReportCard
                  ...
                  actQty={listUsers && listUsers.filter(user => user.active === true).length}
                />
              </Col>
              <Col sm={6}>
                {listProducts &&
