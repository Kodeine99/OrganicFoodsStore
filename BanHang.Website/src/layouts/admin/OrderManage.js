...
function OrderManage(props) {
  ...
  const onLoadData = async (values) => {
    const actionResult = await dispatch(getAllOrder({...values, PageIndex: pageIndex, PageSize:10}))
    ...
    const usernameItems = items.map(item => ({
      value: item.userName,
      label: item.userName
    }));
        ...
        <Container fluid>
          ...
          <Row className={'m-b-20 justify-content-between m-t-10'}>
            ...
            <h6 className="d-block my-auto mx-0">
              ...
              <span className="text-primary">{pagination.totalRecords}</span> results
            </h6>
            }
            <CustomPagination
